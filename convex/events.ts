...
// Helper function to check ticket availability - this is internal logic, not a query
async function checkTicketAvailability(ctx: any, eventId: string) {
    ...
    const activeOffers = await ctx.db
        ...
        .collect()
        .then(
            (entries: any[]) => entries.filter((e) => (e.offerExpiresAt ?? 0) > now).length
        );
...
// Public query that uses the helper function
...
export const joinWaitingList = mutation({
    ...
    handler: async (ctx, { eventId, userId }) => {
        ...
        // Check if there are any available tickets right now using the helper function
